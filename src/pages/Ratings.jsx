...
export default function Ratings() {
  ...
  return (
    <section ref={ref} className="w-full bg-gray-50 py-16 md:py-20 px-4 sm:px-6">
      ...
      <motion.div className="max-w-6xl mx-auto grid gap-6 sm:gap-8 md:gap-10 sm:grid-cols-2 lg:grid-cols-3" variants={containerVariants} initial="hidden" animate={isInView ? "visible" : "hidden"}>
        {reviews.map((review, index) => (
          <motion.div key={index} variants={itemVariants} className={`${review.color} p-6 md:p-8 rounded-2xl shadow-md hover:shadow-xl transition-shadow duration-300 flex flex-col justify-between`}>
            <div>
              <div className="flex items-center gap-4 mb-4">
                <img src={review.image} alt={review.name} className="w-12 h-12 md:w-14 md:h-14 rounded-full object-cover border-2 border-white/20" />
                <div>
                  <h3 className="font-semibold text-white text-sm md:text-base">{review.name}</h3>
                  <p className="text-xs md:text-sm text-gray-100">{review.country}</p>
                </div>
              </div>
              ...
              <p className="text-white text-sm md:text-base leading-relaxed italic">{review.text}</p>
            </div>
          </motion.div>
        ))}
      </motion.div>
