...
export default function Tours() {
  ...
  useEffect(() => {
    const fetchTours = async () => {
      setLoading(true);
      try {
        const response = await axios.get("http://127.0.0.1:8000/api/tours", {params: { destination, duration }});
          ...
          tours.map((tour) => (
            <div key={tour.id} className="bg-white rounded-2xl shadow-md hover:shadow-xl transition p-4 flex flex-col gap-4">
              <figure className="w-full h-48 overflow-hidden rounded-xl">
                <img src={tour.image_url || "https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1"} alt={tour.title} className="w-full h-full object-cover" />
              </figure>
              ...
              <div className="flex justify-between items-center mt-auto">
                ...
                <Link to={`/tours/${tour.id}`} className="bg-blue-700 text-white px-6 py-2 rounded-xl hover:bg-blue-600 transition">View Details</Link>
              </div>
            </div>
          ))) : (<div className="col-span-full text-center py-10 text-gray-400">No tours found matching your search.</div>)
        }
      </section>
    </main>
  );
